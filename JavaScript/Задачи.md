...
// Используем Map для хранения уже просмотренных чисел и их индексов
const map = new Map();

for (let i = 0; i < nums.length; i++) {
	// Вычисляем, какое число нужно найти, чтобы получить target
	const complement = target - nums[i];
	// Если нужное число уже есть в Map — возвращаем индексы
...
return arr.filter((el) => el.search(new RegExp(`${substring}`)) > -1);

return arr.filter((el) =>  
  el.indexOf(substring.charAt(0)) > -1 &&  
  el.slice(
	  el.indexOf(substring.charAt(0)), 
	  el.indexOf(substring.charAt(0)) + substring.length) === substring  
	  );
```
